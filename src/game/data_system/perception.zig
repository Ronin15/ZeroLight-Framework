...
pub const PerceptionStore = struct {
    ...
    /// and the recomputed cos_half_fov) on an existing row. A stat retune must
    /// not wipe live sensing state, so the hot columns (target_visible,
    /// last_seen_x/y, nearest_threat, nearest_threat_dist, facing_x/y) are
    /// left untouched.
    ...
    /// tunables (vision_range, fov_half_angle_radians, cos_half_fov) stay
    /// const here — they change only through PerceptionStore.set.
...
test "PerceptionStore append/set/get/removeAt round-trip" {
    ...
    _ = try store.append(std.testing.allocator, first, .{ .vision_range = 100, .fov_half_angle_radians = 0.5 });
    _ = try store.append(std.testing.allocator, second, .{ .vision_range = 200, .fov_half_angle_radians = 0.6 });
    _ = try store.append(std.testing.allocator, third, .{ .vision_range = 300, .fov_half_angle_radians = 0.7 });
    try std.testing.expectEqual(@as(usize, 3), store.len());

    store.set(0, .{ .vision_range = 150, .fov_half_angle_radians = 0.9 });
...
test "PerceptionStore set on existing row preserves hot columns and only updates cold columns" {
    ...
    // A stat retune (e.g. tuning vision_range) must not clear the hot state above.
    store.set(0, .{ .vision_range = 400, .fov_half_angle_radians = 0.4 });
