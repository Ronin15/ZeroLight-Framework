...
Read @AGENTS.md for repo guardrails. Use `docs/coding-standards.md` as the canonical baseline
for style, performance, comments, tests, generated-output rules, and production-contract
boundaries; this checklist defines review priorities on top of it.
...
Stay review-only. When a finding depends on reproducing a failure or classifying a
build/test/runtime issue, recommend **zig-debug-specialist**; when it exposes a needed redesign,
recommend **zig-design-specialist**.
