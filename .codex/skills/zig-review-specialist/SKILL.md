...
- Comments and doc comments: exported cross-module APIs should use `///` immediately above declarations when callers need ownership, lifetime, invariants, ordering, threading, allocation, failure, or performance context. Private helpers, implementation phases, local invariants, hot-path rationale, and test fixture notes should use ordinary `//`. Flag comments that sit in the wrong place, restate obvious code, carry stale roadmap intent, or make broad claims not enforced by code or tests.
- Tests: behavior-focused tests should cover pure logic; GPU/display checks should stay separate from ordinary unit coverage. Flag test-only enum tags, union variants, marker fields, fake stages, fixture hooks, service shortcuts, or test-only paths leaking into production contracts in any subsystem.
