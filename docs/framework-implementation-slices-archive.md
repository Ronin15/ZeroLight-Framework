# Framework Implementation Slices — Archive (Slices 0–7, 9–17)

Completed, settled foundation slices split out of
[framework-implementation-slices.md](framework-implementation-slices.md) to keep
the live roadmap focused on the frontier. Every slice here is done and verified
(all items checked); this file is the historical acceptance record. The live
roadmap owns current priorities, the dependency-ordered "Suggested Order", and
Slices 18 onward.

> Slice 8 (Shader And Platform Expansion) stays in the live roadmap, not here,
> because it still has open shader/material hardening items.
